...
// 1) Add lead_score to contacts
if(!column_exists($db,'contacts','lead_score')){
  $db->exec("ALTER TABLE contacts ADD COLUMN lead_score INTEGER DEFAULT 0;");
}
$db->exec("CREATE INDEX IF NOT EXISTS idx_contacts_score ON contacts(lead_score);");

// 2) lead_score_rules
$db->exec("CREATE TABLE IF NOT EXISTS lead_score_rules (
...
// 3) content_posts (Content Scheduler)
