...
global $db; 
layout_header('Giao dịch');

// Xử lý các action
if ($op==='create' && $_SERVER['REQUEST_METHOD']==='POST'){ 
    require_csrf(); 
    q($db, "INSERT INTO deals(contact_id,title,value,currency,stage,channel,utm_source,utm_medium,utm_campaign,expected_close) VALUES(?,?,?,?,?,?,?,?,?,?)", [
    ...
    header('Location: ?action=deals'); 
    exit; 
}

if ($op==='update' && $_SERVER['REQUEST_METHOD']==='POST'){ 
    require_csrf(); 
    q($db, "UPDATE deals SET contact_id=?,title=?,value=?,currency=?,stage=?,channel=?,utm_source=?,utm_medium=?,utm_campaign=?,expected_close=? WHERE id=?", [
        $_POST['contact_id']?:null,
        $_POST['title']??'',
        (float)($_POST['value']??0),
        $_POST['currency']??'USD',
...
if ($op==='delete'){ 
    q($db, "DELETE FROM deals WHERE id=?", [(int)($_GET['id']??0)]); 
    header('Location: ?action=deals'); 
    exit; 
}
...
if ($op==='new' || $op==='edit'){
    ...
    if ($op==='edit') {
        $d = q($db, "SELECT * FROM deals WHERE id=?", [(int)$_GET['id']])->fetch(PDO::FETCH_ASSOC) ?: $d;
    }
    
    $contacts = q($db, "SELECT id,name FROM contacts ORDER BY name")->fetchAll(PDO::FETCH_ASSOC);
    
    echo '<div class="card"><h3>'.($op==='new'?'Thêm giao dịch':'Sửa giao dịch').'</h3><form method="post" action="?action=deals&op='.($op==='new'?'create':'update').'">';
    csrf_field(); 
    if ($op==='edit') echo '<input type="hidden" name="id" value="'.h($d['id']).'">';
    
    echo '<div class="grid cols-3">';
    echo '<div><label>Liên hệ</label><select name="contact_id"><option value="">—</option>';
    foreach($contacts as $c){ 
        $sel=$d['contact_id']==$c['id']?'selected':''; 
        echo '<option '.$sel.' value="'.$c['id'].'">'.h($c['name']).'</option>'; 
    } 
    echo '</select></div>';
    ...
    echo '</div>';
    echo '<div class="hint">Gợi ý: Kanban kéo-thả sẽ bổ sung sau; tạm thời chỉnh Stage tại đây.</div>';
    echo '<div style="margin-top:12px"><button class="btn">Lưu</button> <a class="btn secondary" href="?action=deals">Huỷ</a></div>';
    echo '</form></div>';
}

// Kanban board view cho deals
echo '<div class="grid cols-3">';
foreach(['New','Qualified','Proposal'] as $col){
    ...
    foreach($rows as $r){
        ...
        echo '<a class="btn secondary" href="?action=deals&op=edit&id='.(int)$r['id'].'">Sửa</a> ';
        echo '<a class="btn secondary" href="?action=deals&op=delete&id='.(int)$r['id'].'" onclick="return confirm(\'Xoá giao dịch?\')">Xoá</a>';
        echo '</div>';
        echo '</div>';
    }
    echo '</div>';
...
// Bảng deals đã chốt
echo '<div class="card"><h3>Đã chốt</h3>';
echo '<div style="margin-bottom:8px"><a class="btn" href="?action=deals&op=new">Thêm Deal Mới</a></div>';
echo '<table><tr><th>Tiêu đề</th><th>Liên hệ</th><th>Giá trị</th><th>Stage</th><th>Kênh</th><th>UTM</th><th></th></tr>';
...
foreach($closed as $r){ 
    ...
    echo '<td>'.h($r['currency']).' '.number_format($r['value'],2).'</td>';
    echo '<td>'.h($r['stage']).'</td>';
    echo '<td>'.h($r['channel']).'</td>';
    echo '<td>'.h($utm).'</td>';
    echo '<td><a href="?action=deals&op=edit&id='.(int)$r['id'].'">Sửa</a></td>';
    echo '</tr>'; 
}
echo '</table></div>';
echo '</div>';
