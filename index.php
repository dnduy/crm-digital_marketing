...
require __DIR__.'/lib/marketing.php'; // NEW
...
if ($action === 'api') { header('Content-Type: application/json');
  $what = $_GET['what'] ?? '';
  if ($what==='contacts') { $rows=q($db,"SELECT id,name,email,phone,company,source,tags,utm_source,utm_medium,utm_campaign,lead_score,created_at FROM contacts ORDER BY id DESC LIMIT 1000")->fetchAll(PDO::FETCH_ASSOC); echo json_encode(['contacts'=>$rows],JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES); exit; }
...
// Webhook Lead Intake (scoring + automation)
if ($action === 'webhook') {
  ...
  $is_new = false; $cid = null;
  if ($email) { $cid = q($db,"SELECT id FROM contacts WHERE email=? ORDER BY id DESC LIMIT 1",[$email])->fetchColumn() ?: null; }
  if (!$cid) {
    q($db,"INSERT INTO contacts(name,email,phone,company,source,tags,utm_source,utm_medium,utm_campaign,lead_score) VALUES(?,?,?,?,?,?,?,?,?,0)",
